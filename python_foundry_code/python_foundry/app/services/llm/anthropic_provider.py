...
class AnthropicProvider(BaseLLMProvider):
    ...
    def _chat(
        ...
        requested_model = (self.settings.anthropic_model or "").strip()
        fallback_models = [
        ...
        for model in candidate_models:
            try:
                ...
                last_error = exc
                logger.warning(
                    "Anthropic model unavailable, trying fallback. trace_id=%s model=%s",
                    trace_id,
                    model,
                )
        ...
        if selected_model and selected_model != requested_model:
            logger.info(
                "Anthropic fallback model selected trace_id=%s requested=%s selected=%s",
                trace_id,
                requested_model,
                selected_model,
            )
